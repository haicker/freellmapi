...
function ProviderSettingsRow({
  ...
  const [intelligenceRank, setIntelligenceRank] = useState(
    model.intelligenceRank != null ? String(model.intelligenceRank) : '50'
  )
  ...
  useEffect(() => {
    ...
    setIntelligenceRank(model.intelligenceRank != null ? String(model.intelligenceRank) : '50')
  ...
  }, [model.modelDbId, model.displayName, model.contextWindow, model.supportsVision, model.supportsTools, model.intelligenceRank, model.enabled, model.rpmLimit, model.rpdLimit])
  ...
  const rawRank = intelligenceRank.trim() === '' ? 50 : Number(intelligenceRank)
  const parsedRank = Math.max(1, Math.min(100, Number.isNaN(rawRank) ? 50 : rawRank))
  const rankInvalid =
    !Number.isInteger(rawRank) || rawRank < 1 || rawRank > 100
  const parsedRpm = rpmLimit.trim() === '' ? null : Number(rpmLimit)
  const rpmInvalid = parsedRpm !== null && (!Number.isInteger(parsedRpm) || parsedRpm <= 0)
  const parsedRpd = rpdLimit.trim() === '' ? null : Number(rpdLimit)
  const rpdInvalid = parsedRpd !== null && (!Number.isInteger(parsedRpd) || parsedRpd <= 0)
  ...
  const dirty =
    ...
    parsedRank !== (model.intelligenceRank ?? 50) ||
    parsedRpm !== (model.rpmLimit ?? null) ||
    parsedRpd !== (model.rpdLimit ?? null) ||
    fallbackEnabled !== model.enabled
  const canSave = dirty && !nameInvalid && !contextInvalid && !rankInvalid && !rpmInvalid && !rpdInvalid && !saving && !deleting
  ...
  return (
    <div className="rounded-xl border bg-background/60 p-3">
      ...
      <div className="grid gap-3 md:grid-cols-[minmax(11rem,1fr)_7rem_6rem_6rem_6rem_auto_auto_auto_auto] md:items-end">
        ...
        <label className="space-y-1 text-xs text-muted-foreground">
          ...
          <Input
            ...
            onChange={e => setIntelligenceRank(e.target.value)}
            aria-invalid={rankInvalid}
            className="text-sm tabular-nums w-full"
          />
        </label>
        <label className="space-y-1 text-xs text-muted-foreground">
          <span>{t('models.rpmLimitLabel')}</span>
          <Input
            type="number"
            min={1}
            step={1}
            ...
            aria-invalid={rpmInvalid}
            placeholder="∞"
            className="text-sm tabular-nums w-full"
          />
        </label>
        <label className="space-y-1 text-xs text-muted-foreground">
          <span>{t('models.rpdLimitLabel')}</span>
          <Input
            type="number"
            min={1}
            step={1}
      ...
      <p className="text-[11px] text-muted-foreground/70">{t('models.rateLimitHint')}</p>
    </div>
  )
}
