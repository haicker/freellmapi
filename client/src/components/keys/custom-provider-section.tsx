...
export function CustomProviderSection({ onAdded }: { onAdded?: () => void } = {}) {
  ...
  const baseUrlError = !baseUrl.trim()
    ...
    : !isHttpUrl(baseUrl)
      ? t('validation.url')
      : null
  const modelError = modelRequired && models.length === 0 ? t('validation.required') : null
  ...
  >({
    ...
    onSuccess: (data) => {
      ...
      if (onAdded) {
        const added = (data?.models?.length ?? 0) > 0
        toast.success(added ? t('keys.modelAdded') : t('keys.endpointAdded'))
        onAdded()
      }
    },
  })
  ...
  const form = (
      <form onSubmit={submit} className="flex flex-wrap items-end gap-3">
        ...
        <div className="space-y-1.5">
          ...
          {!modelRequired && (
            <p className="text-[11px] text-muted-foreground">{t('keys.customModelOptional')}</p>
          )}
        </div>
        <div className="space-y-1.5">
