...
import { ChevronDown, ExternalLink, KeyRound, MoreHorizontal, Pencil, Plus, RefreshCw, Search, Trash2, List } from 'lucide-react'
...
export function ProviderList({ onAddKey }: { onAddKey: () => void }) {
  ...
  const [modelSelectionKey, setModelSelectionKey] = useState<ApiKey | null>(null)
  // Track which provider groups have their model list expanded
  const [expandedModelGroups, setExpandedModelGroups] = useState<Set<string>>(new Set())
  const editInputRef = useRef<HTMLInputElement>(null)

  const { data: keysResponse, isLoading } = useQuery<{ keys: ApiKey[]; platformModels: Record<string, ApiKeyModel[]> }>({
    queryKey: ['keys'],
    queryFn: () => apiFetch('/api/keys'),
  })
  const keys = keysResponse?.keys ?? []
  const platformModels = keysResponse?.platformModels ?? {}
  ...
  return (
          ...
          {visibleGroups.map(group => {
            ...
            return (
              <div key={group.value}>
                ...
                {expanded && (
                  <>
                    {/* Platform-level models (non-custom providers) */}
                    {group.value !== 'custom' && platformModels[group.value]?.length > 0 && (
                      <div className="mb-3">
                        <button
                          type="button"
                          onClick={() => {
                        ...
                        >
                          <ChevronDown className={`size-3 transition-transform ${expandedModelGroups.has(group.value) ? '' : '-rotate-90'}`} />
                        ...
                        <div key={k.id} className="bg-card">
                          <div className="group/krow flex items-center gap-3 px-4 py-3 hover:bg-muted/40 transition-colors">
                            ...
                            <div className="flex items-center gap-0.5 opacity-0 transition-opacity group-hover/krow:opacity-100 focus-within:opacity-100 pointer-coarse:opacity-100">
                              ...
                              {k.platform !== 'aihorde' && (
                                <Tooltip text={t('keys.selectModels')}>
                                  <Button
                                    variant="ghost"
                                    size="icon-xs"
                                    onClick={() => setModelSelectionKey(k)}
                                    aria-label={t('keys.selectModels')}
                                  >
    ...
    {modelSelectionKey && (
      <ModelSelectionDialog
        keyData={modelSelectionKey}
        open={!!modelSelectionKey}
        onOpenChange={(open) => !open && setModelSelectionKey(null)}
      />
    )}
    </>
  )
