...
// The purple "intelligence" bar is a DIRECT map of the user's manual 智能值
// (intelligence_rank, 1–100): the value IS the fill — rank 50 → 50%, rank 100 →
// 100%, rank 1 → 1%. No conversion. Higher = smarter (convention flipped 2026-07),
// so it now reads the same way the user edits it on the detail page.
function intelligenceBarValue(row: { intelligence?: number; intelligenceRank?: number }): number {
  const r = Math.max(1, Math.min(100, row.intelligenceRank ?? 50))
  return r / 100
}
...
export function RowContent({
  ...
  return (
    <>
      ...
      <td className="py-2 pr-3 align-middle">
        <Tooltip text={`${t('models.intelligenceRank')}: ${row.intelligenceRank ?? '—'}`}>
          <span><AxisBar value={intelligenceBarValue(row)} color="#a855f7" /></span>
        </Tooltip>
      </td>
...
export function GroupHeaderCells({ group, rank, dragHandle, onToggleGroup }: {
  ...
  return (
    <>
      ...
      <td className="py-2 pr-3 align-middle">
        <Tooltip text={`${t('models.intelligenceRank')}: ${best.intelligenceRank ?? '—'}`}>
          <span><AxisBar value={intelligenceBarValue(best)} color="#a855f7" /></span>
        </Tooltip>
      </td>
