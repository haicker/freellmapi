...
export interface ModelOption {
  ...
  // Sort axes for "by intelligence": the manual 智能值 (intelligence_rank, 1–100)
  // is a direct smartness score — HIGHER = smarter (convention flipped 2026-07) —
  // so the dashboard orders by it directly; the size tier no longer influences
  // ordering. For a group these hold the BEST (smartest) member's values.
  sizeTier: number     // Frontier=1, Large=2, Medium=3, Small=4, unknown=5 (display only)
  intelligenceRank: number
}
...
export function buildModelOptions(entries: PickerEntry[], unifyOn: boolean): ModelOption[] {
  ...
  // group, keep the SMARTEST member (highest 智能值) so "sort by intelligence"
  // ranks the group by its most capable provider, and collect every provider name.
  const groups = new Map<string, ModelOption>()
  for (const e of entries) {
    const key = e.groupKey ?? e.modelId
    const tier = sizeTier(e.sizeLabel)
    const rank = e.intelligenceRank ?? 0
    const existing = groups.get(key)
    if (existing) {
      existing.providerCount++
      existing.platforms.push(e.platform)
      if (rank > existing.intelligenceRank) {
