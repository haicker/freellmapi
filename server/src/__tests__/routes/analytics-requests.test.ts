...
describe('GET /api/analytics/requests', () => {
  ...
  it('returns one row per call, newest first, with caller identity', async () => {
    const oldest = recentTimestamp(3);
    const middle = recentTimestamp(2);
    const newest = recentTimestamp(1);
    insertCall(oldest, '192.168.0.3', 'curl/8.6.0');
    insertCall(middle, '192.168.0.15', 'python-httpx/0.27');
    insertCall(newest, null, null);
    ...
    expect(body.rows[1].createdAt).toBe(`${middle.replace(' ', 'T')}Z`);
  });

  it('paginates with limit/offset and clamps limit to 500', async () => {
    const timestamps = Array.from({ length: 5 }, (_, i) => recentTimestamp(5 - i));
    for (const timestamp of timestamps) insertCall(timestamp, '10.0.0.1', 'ua');
    ...
    expect(page.body.rows[0].createdAt).toBe(`${timestamps[2].replace(' ', 'T')}Z`);
