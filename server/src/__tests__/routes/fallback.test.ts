...
describe('Fallback API', () => {
  ...
  it('POST /api/fallback/sort/intelligence sorts by intelligence score descending', async () => {
    ...
    for (let i = 1; i < body.length; i++) {
      expect(body[i].intelligenceRank).toBeLessThanOrEqual(body[i - 1].intelligenceRank);
    }
  });

  it('intelligence score overrides the display-only size tier', async () => {
    const db = getDb();
    const frontier = db.prepare(
      "SELECT id, intelligence_rank FROM models WHERE size_label = 'Frontier' LIMIT 1",
    ).get() as { id: number; intelligence_rank: number };
    const small = db.prepare(
      "SELECT id, intelligence_rank FROM models WHERE size_label = 'Small' LIMIT 1",
    ).get() as { id: number; intelligence_rank: number };

    try {
      db.prepare('UPDATE models SET intelligence_rank = 10 WHERE id = ?').run(frontier.id);
      db.prepare('UPDATE models SET intelligence_rank = 90 WHERE id = ?').run(small.id);
      await request(app, 'POST', '/api/fallback/sort/intelligence');
      const { body } = await request(app, 'GET', '/api/fallback');
      expect(body.findIndex((m: any) => m.modelDbId === small.id))
        .toBeLessThan(body.findIndex((m: any) => m.modelDbId === frontier.id));
    } finally {
      db.prepare('UPDATE models SET intelligence_rank = ? WHERE id = ?').run(frontier.intelligence_rank, frontier.id);
      db.prepare('UPDATE models SET intelligence_rank = ? WHERE id = ?').run(small.intelligence_rank, small.id);
    }
  });
