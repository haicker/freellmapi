...
// Composite intelligence: the manual 智能值 (intelligence_rank, 1–100) is now a
// DIRECT smartness score — HIGHER = smarter — and drives routing with NO
// size_label tier boost, so editing the value moves routing proportionally for
// every model (not just within a tier). Clamp to 1–100 so any legacy >100 value
// can't dominate the chain. (Convention flipped 2026-07; was tier*1000 - rank.)
function intelligenceComposite(_sizeLabel: string, intelligenceRank: number): number {
  const r = Math.max(1, Math.min(100, intelligenceRank ?? 50));
  return r;
}
 ...
 */
/**
 * Select a usable API-key-backed route for a model (the normal, keyed path).
 * Returns null when every keyed option is unavailable right now (all cooled
 * down, over quota, undecryptable, status not healthy/unknown, or no key row
 * at all). `diag`, when provided, accumulates the per-key/per-model reason for
 * the failure so the exhaustion error can report *why* (issue _1).
 */
function selectRealKeyForModel(
  entry: ChainRow,
  provider: BaseProvider,
...
function selectKeyForModel(entry: ChainRow, estimatedTokens: number, skipKeys?: Set<string>, diag?: string[]): RouteResult | null {
  ...
  return selectRealKeyForModel(entry, provider, estimatedTokens, skipKeys, diag);
}
...
export function resolveFusionCandidate(modelId: string): FusionCandidate | null {
  ...
  const row = db.prepare(`
    ...
    ORDER BY m.intelligence_rank DESC, m.id ASC
    LIMIT 1
  `).get(modelId) as {
