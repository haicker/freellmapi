...
export function groupRows(rows: GroupableRow[], ov: UnifyOverrides): ModelGroup[] {
  ...
  // Representative label = the smartest (HIGHEST intelligence_rank) member,
  // tiebroken by shortest stripped name then model_db_id, so the label is
  // deterministic. (Convention flipped 2026-07: higher 智能值 = smarter.)
