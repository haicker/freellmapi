...
export class OpenAICompatProvider extends BaseProvider {
  ...
  async getAvailableModels(apiKey: string): Promise<Array<{id: string; name: string; supportsTools?: boolean; supportsVision?: boolean; free?: boolean}> | undefined> {
    try {
      ...
      // hang the discover-models and add-models routes forever (#hang).
      ...
      // the user the upstream was too slow, instead of a generic "failed".
      const err = error as Error & { name?: string };
      if (err?.name === 'AbortError' || /aborted|timeout/i.test(err?.message ?? '')) {
        throw new Error(`Model discovery timed out after ${Math.round(this.timeoutMs / 1000)}s (provider ${this.platform} did not respond)`);
      }
      console.warn(`[${this.platform}] Failed to fetch models:`, error);
      return undefined;
    }
  }
}
