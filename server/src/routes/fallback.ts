...
function getBudgetScore(m: { tpd_limit: number | null }): number {
  // Rank by throughput capacity (tokens per day). Higher tpd_limit = more capacity.
  return m.tpd_limit ?? 0;
}
...
fallbackRouter.post('/sort/:preset', (req: Request, res: Response) => {
  ...
  if (preset === 'budget') {
    const allModels = db.prepare(`SELECT id, tpd_limit FROM models`).all() as any[];
...
fallbackRouter.get('/token-usage', (_req: Request, res: Response) => {
  ...
  let rawModels: { model_db_id: number; platform: string; model_id: string; display_name: string; priority: number; enabled: number; rpm_limit: number | null; rpd_limit: number | null; tpm_limit: number | null; tpd_limit: number | null }[];
